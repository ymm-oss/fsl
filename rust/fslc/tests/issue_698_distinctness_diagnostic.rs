...
/// REJECTING CONTROL for the independent review's counterexample: the affine
/// offset is a conditional. `expr_text` does not parenthesize one, so splicing
/// it raw into `k >= <offset>` and `k < <offset> + n` reassociates the
/// comparison. The emitted hint must keep the offset bracketed.
#[test]
fn a_conditional_offset_is_parenthesised_in_the_hint() {
    let path = format!("{FIXTURE_DIR}/issue_698_conditional_offset.fsl");
    let (output, status) = run_cli(&["check", &path]);
    assert_eq!(status, 2, "{output:#}");
    assert_eq!(
        output["diagnostic_code"], "FSL-SEMANTIC-WRITE-DISTINCTNESS-UNPROVED",
    ...
    let hint = output["hint"].as_str().expect("hint");
    assert!(
        hint.contains("k >= (if flag then 1 else 2)"),
        "the offset must be bracketed where it is compared: {hint}"
    );
    assert!(
        hint.contains("k < (if flag then 1 else 2) + 3"),
        "the offset must be bracketed where it is added to: {hint}"
    );
}
