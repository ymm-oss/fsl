...
if PYTHONPATH= "$VENV_PYTHON" -m pip install "$REPO_DIR" >"$PIP_INSTALL_LOG" 2>&1; then
  ...
  rm -rf "$SITE_PACKAGES/fslc" "$SITE_PACKAGES/fslc-1.0.3.dist-info"
  cp -R "$REPO_DIR/src/fslc" "$SITE_PACKAGES/fslc"
  mkdir -p "$SITE_PACKAGES/fslc-1.0.3.dist-info"
  {
    echo "Metadata-Version: 2.1"
    echo "Name: fslc"
    echo "Version: 1.0.3"
  } > "$SITE_PACKAGES/fslc-1.0.3.dist-info/METADATA"
  {
    echo "fslc"
  } > "$SITE_PACKAGES/fslc-1.0.3.dist-info/top_level.txt"
  cat > "$FSL_BIN" <<EOF
#!$VENV_PYTHON
