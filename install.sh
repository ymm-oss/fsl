...
INSTALL_SKILL=1
CLONE_URL="${FSL_INSTALL_REPO_URL:-https://github.com/yumemi/fsl}"
...
if [ -z "$REPO_DIR" ]; then
  ...
  if [ -e "$INSTALL_DIR" ]; then
    ...
    git clone "$CLONE_URL" "$INSTALL_DIR" || fail "FSL リポジトリの取得に失敗しました。ネットワーク接続と git の設定を確認してから再実行してください。"
  fi
...
if PYTHONPATH= "$VENV_PYTHON" -m pip install "$REPO_DIR" >"$PIP_INSTALL_LOG" 2>&1; then
  ...
  rm -rf "$SITE_PACKAGES/fslc" "$SITE_PACKAGES/fslc-0.0.1.dist-info"
  cp -R "$REPO_DIR/src/fslc" "$SITE_PACKAGES/fslc"
  mkdir -p "$SITE_PACKAGES/fslc-0.0.1.dist-info"
  {
    echo "Metadata-Version: 2.1"
    echo "Name: fslc"
    echo "Version: 0.0.1"
  } > "$SITE_PACKAGES/fslc-0.0.1.dist-info/METADATA"
  {
    echo "fslc"
  } > "$SITE_PACKAGES/fslc-0.0.1.dist-info/top_level.txt"
  cat > "$FSL_BIN" <<EOF
#!$VENV_PYTHON
