...
| `refinement_failed` | `errors/refinement_failed_map.fsl` | `abs_requires_failed` when an implementation bypasses an abstract guard |
...
"full-unfolding deadlock → vacuous `refines`" bug. Incremental prefix unfolding
...
4. If the expectation and the actual output differ, do not fix the spec to match
   the observation; add a regression case and update the relevant `DESIGN-*`
