...
def _render_operand(expr, display_names, min_prec):
    ...
    looser. This is the fix for `_expr_to_text` silently dropping
    semantically necessary parens — e.g. `not (A and B)` rendering as
    `not A and B`, which reads as `(not A) and B`."""
    text = _expr_to_text(expr, display_names)
...
def _expr_to_text(expr, display_names=None):
    ...
    if tag == "index":
        ...
        return f"{base}[{_expr_to_text(expr[2], display_names)}]"
    if tag == "field":
        base = _render_operand(expr[1], display_names, _PREC_ATOM)
    ...
    if tag == "count":
        ...
        return f"count({expr[1]}: {domain} where {_expr_to_text(expr[3], display_names)})"
    if tag == "sum":
        domain = _type_ref_to_text(("name", expr[2]))
