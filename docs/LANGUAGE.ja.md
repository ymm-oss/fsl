...
inline `implements` が `check` / `verify` の top-level `result` へ伝播するため列挙しています。同じ `2` の
...
- 組み込みの `@kind(id, text?)` を使って、宣言を 1 行で分類しつつ説明します。
  ...
  lowering を一切変更しません。明示的に `annotations_for` を呼ぶプロセス内の
  消費者向けに、検査済み `KernelModel` へ保持されます。現在の JSON envelope、
  LSP index、監査台帳は汎用アノテーションを投影しません。
- 分類にうまく当てはまらない短い根拠には、推奨されるカスタム名前空間
  `@doc.rationale("...")` を使います(通常の `Custom` アノテーション。13.1 を
  参照)。他のカスタム名前空間と同じ、検証に無害な検査済みモデル表現を持ち、
  文法変更は不要です。公開 JSON、LSP、監査の消費者がこの根拠をクエリするには、
  明示的な投影契約が別途必要です。
...
- `implements` があると、`fslc check` と `fslc verify` はどちらも上位レイヤーへの refinement も
  ...
  恒等の refinement を自動生成します。`verify` が `--property`、`--exclude-properties`、
