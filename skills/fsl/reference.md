...
spec <Name> {
  ...
  invariant <Name> { <式> }
  trans     <Name> { <式> }              // 2状態安全性。old(式) で旧状態
  reachable <Name> { <式> }
...
compose <Name> {
  ...
  invariant/trans/reachable/leadsTo ...          // alias.var で横断参照
}
```
...
- ensures/trans 限定: `old(式)` / leadsTo 限定: `P ~> Q` / 写像式限定: `if c then a else b`
...
デッドロック(warning、状態を含む `deadlock reachable at step N (state: …)`、
`--deadlock error` で violated)/ leadsTo(ラッソ+停滞)。
**意図した終端状態**(処理完了など、そこで止まるのが正しい状態)は `terminal { <述語> }`
で宣言する — 述語を満たす停止はデッドロック検査から除外され、それ以外の予期せぬ
デッドロックは引き続き検出される(`--deadlock ignore` が全停止を一律無視するより精密)。
...
継承複製で、成分 spec 単体の verify で検査される)、**どのアクションも代入しない
frozen な state 変数だけに依存し動的に恒真な invariant**(`tautology_over_frozen` —
死んだゴースト。`const` 化するか、変更すべきアクションの欠落を疑う)。
...
- `check` / `verify` の `--strict-tags` は ok/verified/proved の成功結果だけに
  トレーサビリティ warning を追加する。対象はタグなし action/invariant/trans/
  reachable/leadsTo と、`--requirements ids.txt` または requirements 方言の `requirement`
...
invariant / trans / reachable / leadsTo / action の `{` 直前に `"ID: 原文"`:
