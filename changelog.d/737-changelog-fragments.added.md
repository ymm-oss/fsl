...
also rejecting a conforming-looking name hidden in a subdirectory, which
earlier releases of this fragment left invisible to every check but
control 1's own shallow path match), duplicate (id, category),
...
fragment (now with a release exclusion exactly as narrow as the release
shape itself -- product-surface changes and fragment deletions in one
diff -- so a release pull request can merge, and now also catching a
fragment emptied by edit, not only one added empty), and aggregation
...
Fragment bodies are also checked for hygiene (CRLF, a leading list or
heading marker) before aggregation. The category vocabulary is eleven
